			...
			: base( ownerType )
		{
			...
			if ( !EnumType.IsEnum )
			{
				throw new ArgumentException(
					"This factory requires an enum identifying the instances to create, which should be passed as the template parameter." );
			}
				...
				if ( EnumValues.Length != enumsUsed )
				{
					throw new ArgumentException(
						"Not all enum values of the template parameter type \"" + typeof( TEnum )
							+ "\" have a class member with a matching attribute ID set in the type '" + OwnerType.Name + "'." );
				}
			}
		}
